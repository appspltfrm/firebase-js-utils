 ...
 */
export async function getFilteredDataFromPipeline({ pipeline, filters, querySort, limit, queryOffset, joinValues, transliterate }) {
    ...
    const records = (await executePipeline(pipe)).results.map(result => result.data());
 ...
 */
export function buildClientJoinCondition(filter, values, expr) {
 ...
 */
export function buildAdminJoinCondition(filter, expr, transliterate) {
    const P = Firestore.admin().Pipelines;
...
//#endregion
//# sourceMappingURL=_getFilteredDataFromPipeline.js.map