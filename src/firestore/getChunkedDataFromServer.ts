...
export function getChunkedDataFromServer<T = DocumentData>(query: Query<T>, chunkSize: number): AsyncGenerator<T[]>;

export async function* getChunkedDataFromServer<T = DocumentData>(query: Query<T>, chunkSize: number): AsyncGenerator<T[], void, undefined> {
    let done = false;
