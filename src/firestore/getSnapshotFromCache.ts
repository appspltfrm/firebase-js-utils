...
export async function getSnapshotFromCache<T extends DocumentData = any>(query: QueryClient<T>): Promise<QuerySnapshotClient<T>>;

export async function getSnapshotFromCache<T extends DocumentData = any>(query: QueryAdmin<T>): Promise<QuerySnapshotAdmin<T>>;

export async function getSnapshotFromCache<T extends DocumentData = any>(query: Query<T>): Promise<QuerySnapshot<T>>;

export async function getSnapshotFromCache<T extends DocumentData = any>(doc: DocumentReferenceClient<T>): Promise<DocumentSnapshotClient<T>>;

export async function getSnapshotFromCache<T extends DocumentData = any>(doc: DocumentReferenceAdmin<T>): Promise<DocumentSnapshotAdmin<T>>;

export async function getSnapshotFromCache<T extends DocumentData = any>(doc: DocumentReference<T>): Promise<DocumentSnapshot<T>>;
